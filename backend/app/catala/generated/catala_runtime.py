...
class Date:
    ...
    def __init__(self, value: Union [Tuple[int, int, int], Date], pos: SourcePosition | None = None):
        try:
            if isinstance(value, Date):
                self.value = value
            else:
                self.value = Date(year=value[0], month=value[1], day=value[2])
        except InvalidDate: raise InvalidDate(pos)

    def __add__(self, other: Duration, round: DateRounding = DateRounding.AbortOnRound,  pos: SourcePosition | None = None) -> Date:
        try: return Date(self.value.__add__(other.value, round))
        except AmbiguousComputation: raise AmbiguousDateRounding(pos)

    @overload
    def __sub__(self, other: Date) -> Duration: ...

    @overload
    def __sub__(self, other: Duration, round: DateRounding = DateRounding.AbortOnRound, pos: SourcePosition | None = None) -> Date: ...

    def __sub__(self, \
                other: Union[Date, Duration], \
                round: DateRounding = DateRounding.AbortOnRound, \
...
class Duration:
    ...
    def __init__(self, value: Union [Tuple[int, int, int], Period]):
        if isinstance(value, Period):
            self.value = value
        else:
            self.value = Period(years=value[0], months=value[1], days=value[2])
