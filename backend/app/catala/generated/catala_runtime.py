...
class Date(Value):
    ...
    def __add__(self, other: Duration, round: DateRounding = DateRounding.AbortOnRound,  pos: SourcePosition | None = None) -> Date:
        ...
        except AmbiguousComputation: raise DateError(pos, note = "ambiguous date computation with no rounding mode specified")
...
class Duration(Value):
    ...
    def __new__(cls, value: Union [Duration, Tuple[int, int, int], Period, int], *args) -> Duration:
        if isinstance(value, Duration):
            return value
        if isinstance(value, Period):
            p = value
        elif isinstance(value, int):
            p = Period(years=value, months=args[0], days=args[1])
        else:
            p = Period(years=value[0], months=value[1], days=value[2])
